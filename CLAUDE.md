...
- `app/agent/graph.py` - LangGraph ReAct agent. `build_agent(model, checkpointer)` returns a compiled graph. `run_agent(question, ...)` is the main entry point. The `tools` node is `_guarded_tool_node` (not bare `ToolNode`) — it validates each tool call via `app/agent/tool_guard.py` before dispatch; blocked calls return a `ToolMessage` error so the agent can reason about the refusal without crashing. Trace shape: each reasoning iteration creates a `ChatOpenAI` observation; tool executions sit between them as `tools` → `<tool_name>` observations. A 3-tool query produces 4 `ChatOpenAI` spans and 3 `tools` nodes under a `LangGraph` root.
- `app/agent/tool_guard.py` - Pre-execution tool-call guardrail. `validate_tool_call(tool_name, tool_args)` enforces an allowlist of permitted tools, checks `search_docs` queries for injection patterns (10-pattern subset of `PromptInjectionGuard`), and bounds-checks `list_traces` limit. Raises `ToolCallBlockedError` on violation.
...
- `scripts/red_team.py` - Automated adversarial probing of the guardrail stack. `run_red_team(attack_types, n_variants, model)` generates adversarial prompt variants via LiteLLM (guardrails bypassed during generation via `guardrails: []` + `x-agentguard-internal` metadata to prevent recursion) then probes each through the full guardrail stack. Four attack types: `prompt_injection` (instruction override), `jailbreak` (DAN/persona escapes), `pii_probe` (PII extraction), `system_prompt_leak` (system prompt exfiltration). HTTP 400/403 = blocked = PASS; HTTP 200 = leaked = FAIL; timeout = leaked (fail-safe). Exit codes: 0=all blocked, 1=any leaked, 2=error — same pattern as `regression_gate.py` for CI integration. Run directly with `python -m scripts.red_team` or via CLI with `python -m app.main red-team`.
...
Three LiteLLM custom guardrails are registered. Two run on every request by default:
- **Prompt injection** (pre_call, always on): blocks 12 regex patterns (jailbreak, ignore instructions, DAN, system prompt exfiltration, etc.). The DAN pattern uses `(?-i:DAN)\b` for case-sensitive matching to avoid false-positiving on the name "Dan". Optional LLM-judge semantic second pass (`SEMANTIC_GUARD_ENABLED=true`) catches paraphrased jailbreaks the regex misses — calls back through LiteLLM with classifier content embedded in the system role to bypass built-in content filters.
- **Toxic content** (pre_call, opt-in via `TOXICITY_GUARD_ENABLED=true`): LLM-judge classifier for toxic/abusive inputs. Also embeds content in system role for the same reason.
...
All containers have log rotation (`10m` max, 2 files). Memory-heavy services have explicit CPU/memory limits via YAML anchors (`x-deploy-clickhouse`, `x-deploy-postgres`, `x-deploy-ollama`). Total Docker memory budget is ~15 GB. Portainer (`:9443`) provides container management UI. Real-time log streaming via Loki (in the infra stack) — query with LogQL in Grafana or via `http://localhost:3101`.
