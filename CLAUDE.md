...
- `app/rag/chain.py` - LCEL chain. `query()` is the main entry point. `ScoredRetriever(BaseRetriever)` replaces the default `VectorStoreRetriever`: calls `similarity_search_with_score()`, injects `retrieval_score` into each `doc.metadata`, and sets four OTel span attributes (`retrieval.chunk_count`, `retrieval.min_score`, `retrieval.max_score`, `retrieval.avg_score`) on the active Langfuse SDK span. `format_docs()` includes the score in the source label sent to the LLM. The attributes land on the `ScoredRetriever` span in Jaeger (not the root HTTP span) because the Langfuse SDK OTel spans are the active context inside `_get_relevant_documents`.
