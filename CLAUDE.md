...
**Open WebUI → Langfuse session linking.** Open WebUI sends the chat UUID in a `chat-id` HTTP header on every `/v1/chat/completions` request. `app/api.py` captures this header and wraps the RAG chain invocation with `propagate_attributes(session_id=chat_id)` from the Langfuse SDK. This stamps the Open WebUI chat ID as `session_id` on every Langfuse trace, so all turns of a conversation are grouped in Langfuse Sessions. Navigation: the share URL `http://localhost:3001/s/<chat-id>` maps directly to `http://localhost:3000/project/my-project/sessions/<chat-id>` — the UUID is the same.
