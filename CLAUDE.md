...
- `openobserve/setup_alerts.sh` - Creates 3 scheduled alerts in OpenObserve via v2 API: `agentguard-error-rate-spike` (≥5 ERROR spans/5min), `agentguard-high-llm-latency` (avg ChatOpenAI duration >30s/10min), `agentguard-guardrail-block-spike` (≥3 failed RunnableSequence/5min). All fire to `agentguard-webhook` destination. Script is idempotent. Set `ALERT_WEBHOOK_URL=https://...` before running to point at Slack/Discord/custom endpoint.
