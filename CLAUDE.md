...
- `docker-compose.monitoring.yml` - Optional monitoring stack (spin up on demand). Contains: jaeger, otel-collector, dozzle, portainer, prometheus, grafana. Uses `networks: langfuse: external: true` — joins the main stack's bridge network so all services are reachable by container name. Start with `docker compose -f docker-compose.monitoring.yml up -d`. When down, OTel spans from the app fail silently (non-fatal; disable entirely with `OTEL_ENABLED=false`).
